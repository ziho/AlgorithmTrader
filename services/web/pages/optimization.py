...
def _new_optimization():
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[600px] max-h-[90vh] overflow-auto"):
                ...
                if strategy and strategy in STRATEGY_PARAM_SPACES:
                    ...
                    for name, spec in params.items():
                        if spec["type"] == "int":
                            ...
                            ui.label(f"{name}: {spec['min']} ~ {spec['max']}, step={step}").classes("text-xs font-mono")
                            total_combinations *= count
                        elif spec["type"] == "float":
                            ui.label(f"{name}: {spec['min']} ~ {spec['max']}").classes("text-xs font-mono")
                            total_combinations *= 10  # 估算
                        elif spec["type"] == "bool":
                            ui.label(f"{name}: True/False").classes("text-xs font-mono")
                            total_combinations *= 2
                    ui.label(f"预计组合数: {total_combinations}").classes("text-xs text-gray-500 mt-2")
        ...
        with ui.row().classes("w-full gap-4 mb-4"):
            weight_sharpe = ui.number(label="夏普", value=0.5, min=0, max=1, step=0.1).classes("flex-1")
            weight_return = ui.number(label="收益", value=0.25, min=0, max=1, step=0.1).classes("flex-1")
            weight_drawdown = ui.number(label="回撤", value=0.25, min=0, max=1, step=0.1).classes("flex-1")
        ...
        with ui.row().classes("w-full gap-4 mb-4"):
            train_days = ui.number(label="训练期(天)", value=180, min=30, max=365).classes("flex-1")
            test_days = ui.number(label="测试期(天)", value=30, min=7, max=90).classes("flex-1")
            n_splits = ui.number(label="分割数", value=6, min=2, max=12).classes("flex-1")
...
def _render_task_row(task: dict):
    ...
    with ui.row().classes(
        ...
        with ui.column().classes("flex-1 gap-0"):
            ui.label(f"{config.get('strategy', 'Unknown')} - {task.get('id', '')}").classes("font-medium")
            ui.label(
        ...
        if status in ("pending", "running"):
            ui.button(icon="cancel", on_click=lambda: None).props("flat dense color=negative")
...
def _render_optimization_results():
    ...
    if not results:
        with ui.card().classes("card w-full"):
            ...
            ui.label("点击「新建优化任务」开始参数优化").classes("text-xs text-gray-400 text-center")
        return
    ...
    with ui.card().classes("card w-full mb-4"):
        ...
        with ui.row().classes("w-full justify-between items-center mb-4"):
            with ui.column().classes("gap-0"):
                ui.label(f"{config.get('strategy', 'Unknown')}").classes("text-lg font-medium")
                ui.label(
...
def _show_result_detail(result: dict):
    """显示优化结果详情"""
    with ui.dialog() as dialog, ui.card().classes("min-w-[800px] max-h-[90vh] overflow-auto"):
        config = result.get("config", {})
        
        ui.label(f"优化结果详情 - {config.get('strategy', '')}").classes("text-lg font-medium mb-4")
        ...
        with ui.card().classes("w-full bg-gray-50 dark:bg-gray-800 p-4 mb-4"):
            ...
            ui.label(f"数据范围: {config.get('start_date')} ~ {config.get('end_date')}").classes("text-sm")
        ...
        if top_results:
            columns = [
                ...
                {"name": "objective", "label": "目标值", "field": "objective", "sortable": True},
                {"name": "sharpe", "label": "夏普", "field": "sharpe", "sortable": True},
                {"name": "return", "label": "收益", "field": "return", "sortable": True},
                {"name": "drawdown", "label": "回撤", "field": "drawdown", "sortable": True},
            ]
