...
def render():
    ...
    with ui.row().classes("w-full justify-between items-center mb-4"):
        ...
        with ui.row().classes("gap-2"):
            ...
            ui.button("刷新", icon="refresh", on_click=lambda: ui.navigate.reload()).props(
                "flat"
            )
...
def _render_strategy_row(strategy: dict):
    ...
    with ui.row().classes(
        ...
        # 交易对
        symbols_text = ", ".join(strategy["symbols"]) if strategy["symbols"] else "未配置"
        symbols_class = "text-base" if strategy["symbols"] else "text-base text-gray-400 italic"
            ...
            if strategy["position"]:
                for symbol, qty in strategy["position"].items():
                    if qty != 0:
                        ...
                        ui.label(f"{symbol}: {qty:+.4f}").classes(f"text-sm {qty_class}")
            else:
...
def _add_strategy_dialog():
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[500px]"):
        ...
        # 策略选择
        strategy_options = [
            {"label": s["class_name"], "value": s["class_name"]}
            for s in available
        ]
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
            ...
            def save_new_strategy():
                ...
                default_params = config_manager.get_default_params(selected_strategy.value)
...
def _view_logs(strategy: dict):
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[700px] max-h-[80vh]"):
        ...
        with ui.scroll_area().classes("w-full h-96 bg-gray-100 dark:bg-gray-800 rounded p-4"):
            # 示例日志
            logs = [
                {"time": "10:30:15", "level": "INFO", "msg": f"Bar received - BTC/USDT close=42350.5"},
                {"time": "10:30:15", "level": "INFO", "msg": "Signal: HOLD (fast_ma=42100, slow_ma=42200)"},
                {"time": "10:15:12", "level": "INFO", "msg": "Bar received - BTC/USDT close=42280.0"},
                {"time": "10:15:12", "level": "INFO", "msg": "Signal: HOLD (fast_ma=42080, slow_ma=42180)"},
                {"time": "10:00:10", "level": "INFO", "msg": "Bar received - BTC/USDT close=42150.0"},
                {"time": "10:00:10", "level": "WARN", "msg": "Signal: BUY - Golden cross detected"},
                {"time": "10:00:11", "level": "INFO", "msg": "Order placed: BUY 0.5 BTC/USDT @ market"},
                {"time": "10:00:12", "level": "INFO", "msg": "Order filled: 0.5 BTC/USDT @ 42155.0"},
            ]
            ...
            if not logs:
                ...
                for log in logs:
                    ...
                    with ui.row().classes("gap-2"):
                        ui.label(f"[{log['time']}]").classes("text-sm font-mono text-gray-400")
                        ui.label(log["level"]).classes(f"text-sm font-mono font-medium {level_class}")
