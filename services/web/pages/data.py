...
def _render_data_stats():
    """渲染数据统计卡片 — 仅统计加密货币数据 (binance + okx)，使用缓存"""
    parquet_dir = PROJECT_ROOT / "data" / "parquet"

    # 使用缓存获取加密货币统计（binance + okx）
    from services.web.parquet_cache import get_parquet_cache

    cache = get_parquet_cache()
    stats = cache.get_crypto_stats(parquet_dir)
    ...
    cards = [
        (
            "📊 数据集",
            f"{stats.datasets}",
            f"{stats.symbols} 个交易对 · {file_count} 个文件",
        ),
    ...
    for title, value, subtitle in cards:
        with ui.card().classes("card flex-1 min-w-48"):
...
def _scan_parquet_datasets() -> list[dict]:
    ...
    # 仅扫描加密货币目录 (binance, okx)
    from services.web.parquet_cache import CRYPTO_DIRS

    for ex_dir in sorted(parquet_dir.iterdir()):
        if not ex_dir.is_dir():
            continue
        ex_name = ex_dir.name  # e.g. "binance", "okx"
        if ex_name not in CRYPTO_DIRS:
            continue
