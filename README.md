# AlgorithmTrader

[![CI](https://github.com/ziho/AlgorithmTrader/actions/workflows/ci.yml/badge.svg)](https://github.com/ziho/AlgorithmTrader/actions/workflows/ci.yml)
[![Python 3.11+](https://img.shields.io/badge/python-3.11+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

一个现代化的个人量化交易系统，专注于中低频策略研发、回测与自动化执行。

适用场景：
- 个人或小团队的量化研究与回测
- 以 15 分钟到 1 小时级别为主的中低频策略
- 需要完整数据采集与回测闭环的研发流程

## 特性

- **统一架构**: 研究、回测、实盘使用相同策略接口，确保结果可复现
- **多数据源**: OKX/ Binance 行情 + Tushare A 股日线与基本面
- **双存储**: Parquet 用于历史数据，InfluxDB 用于实时监控与指标
- **内置策略**: 5 种经典策略开箱即用（双均线、布林带、RSI、通道突破、Z-Score）
- **回测与报告**: 回测结果输出 JSON/Parquet，示例脚本可生成 HTML/Markdown
- **参数优化**: Grid/Random/LHS 搜索 + Walk-Forward 验证
- **A 股支持**: T+1/涨跌停/最小手数规则 + 费用模型 + 因子引擎
- **Web 管理界面**: NiceGUI 控制台（数据下载、回测、优化、服务状态）
- **通知系统**: Telegram / Bark / Webhook / SMTP 邮件
- **容器化部署**: Docker Compose 一键启动基础设施与服务

## 支持的市场

| 市场 | 数据 | 回测 | 实盘 |
|------|------|------|------|
| 加密货币 | OKX / Binance | ✅ | ✅（OKX 现货/永续） |
| A 股 | Tushare 日线 + 基本面 | ✅（A 股规则） | ❌ |
| 美股 | IBKR 占位 | ❌ | ❌ |

**交易所/数据源**: OKX、Binance、Tushare、IBKR(占位)

## 快速开始

### 前置要求

- Python 3.11+
- Docker & Docker Compose
- 16GB+ RAM（推荐 32GB+）

### 安装

```bash
# 克隆项目
git clone https://github.com/ziho/AlgorithmTrader.git
cd AlgorithmTrader

# 安装依赖
pip install -e ".[dev]"

# 配置环境变量
cp .env.example .env
# 编辑 .env 填入你的 API 密钥和配置
```

### 启动基础设施

```bash
# 启动 InfluxDB + Grafana
docker compose up -d influxdb grafana

# 访问 Grafana: http://localhost:3000
# 默认账号: admin / algorithmtrader123
```

### 数据采集

```bash
# OKX 历史数据（默认 15m）
python scripts/demo_collect.py --symbols BTC/USDT,ETH/USDT --days 90

# Binance 公共数据批量下载（1m）
python -m scripts.fetch_history --symbols BTCUSDT,ETHUSDT --from 2020-01-01 --tf 1m
```

### 运行回测

```bash
# 运行示例回测（双均线）
python scripts/demo_backtest.py

# 输出目录: reports/<run_id>/
# 包含: report.html, report.md, summary.json, equity_curve.parquet, trades.parquet
```

## 架构设计

### 核心理念

1. **统一接口**: 策略只关心 `on_bar()` 和 `on_fill()`
2. **数据分层**: 采集 → 清洗 → 特征 → 信号 → 执行
3. **存储分工**:
   - InfluxDB: 实时指标、监控、回测摘要
   - Parquet: 历史数据、回测、特征工程
4. **松耦合服务**: 服务通过存储层协作，便于独立扩展

### 项目结构

```
AlgorithmTrader/
├── src/                    # 核心库（研究/回测/实盘共享）
│   ├── core/               # 基础设施（事件、配置、类型）
│   ├── data/               # 数据层（连接器/抓取/存储/质量）
│   ├── strategy/           # 策略框架与示例策略
│   ├── backtest/           # 回测引擎
│   ├── execution/          # 订单执行与交易所适配
│   ├── portfolio/          # 组合管理与账户核算
│   ├── risk/               # 风控引擎
│   ├── features/           # 因子库与特征工程
│   ├── optimization/       # 参数优化与 Walk-Forward
│   └── ops/                # 运维工具（日志、通知、健康检查）
│
├── services/               # 服务（容器化部署）
│   ├── collector/          # OKX 数据采集服务
│   ├── trader/             # 实盘交易服务
│   ├── scheduler/          # 调度与健康检查
│   ├── backtest_runner/    # 批量回测
│   ├── notifier/           # 消息通知服务
│   └── web/                # NiceGUI 管理界面
│
├── infra/                  # 基础设施配置（Grafana/Loki）
├── scripts/                # 数据下载/回测/工具脚本
├── docs/                   # 文档
└── tests/                  # 测试
```

## 内置策略

| 策略 | 类型 | 说明 |
|------|------|------|
| 双均线交叉 | 趋势跟踪 | 快线上穿慢线做多，死叉平仓 |
| 通道突破 | 趋势跟踪 | 突破N日高点做多，跌破N日低点平仓 |
| 布林带 | 均值回归 | 价格触及下轨做多，上轨平仓 |
| RSI | 均值回归 | 超卖做多，超买平仓 |
| Z-Score | 均值回归 | 基于统计偏离度的配对交易 |

查看 [策略开发指南](docs/guides/strategy_development.md) 了解如何创建自定义策略。

## 技术栈

| 组件 | 技术选型 | 用途 |
|------|---------|------|
| 语言 | Python 3.11+ | 研发与生产一致 |
| 数据存储 | Parquet + DuckDB | 历史数据与分析 |
| 实时监控 | InfluxDB | 指标与监控数据 |
| 交易所接口 | ccxt | OKX 交易执行 |
| Web UI | NiceGUI | 管理控制台 |
| 通知 | Telegram / Bark / Webhook / SMTP | 事件推送 |
| 测试 | pytest | 单元/集成测试 |
| CI/CD | GitHub Actions | 自动化测试 |

## 数据流

```
Binance Public Data ─┐
OKX API ─────────────┼─> Parquet (历史)
Tushare ─────────────┘          │
                               │
                           回测/研究
                               │
OKX 实盘 ──> Trader ────────────┘

实时监控: Collector/Backtest → InfluxDB → Grafana
```

## 开发路线

说明：路线图为方向性描述，实际完成度以 `docs/KNOWN_LIMITATIONS.md` 为准。

### Phase 1: 核心框架（已完成）
- [x] 项目结构与配置管理
- [x] 数据采集与存储
- [x] 回测引擎（单策略 + 批量）
- [x] 5种内置策略
- [x] 风控引擎
- [x] 通知系统
- [x] 特征引擎

### Phase 2: Web 管理界面（进行中）
- [x] 回测结果可视化
- [x] 策略配置管理
- [x] 服务状态监控
- [x] 参数优化框架
- [ ] 实时数据刷新
- [ ] 任务进度展示

### Phase 3: 实盘交易（进行中）
- [x] OKX 现货/永续下单
- [x] 订单管理与追踪
- [x] 滑点与手续费模型
- [ ] 实盘风控完善
- [ ] 异常恢复机制

### Phase 4: 高级功能（规划中）
- [ ] Walk-forward 优化
- [ ] 多策略组合管理
- [ ] Qlib 框架集成

### Phase 5: 其他市场（远期）
- [ ] IBKR 接口（美股）
- [ ] A 股数据对接
- [ ] 期权策略

查看 Issues 了解详细进度。

## 文档

- [文档入口](docs/README.md) - 统一导航入口
- [用户指南](docs/guides/user_guide.md) - 系统使用完整指南
- [策略开发](docs/guides/strategy_development.md) - 如何编写自定义策略
- [部署指南](docs/guides/deployment.md) - 生产环境部署
- [架构设计](docs/development/architecture.md) - 系统架构详解
- [功能完成度](docs/KNOWN_LIMITATIONS.md) - 各模块状态与已知限制

## 快速验证

运行烟雾测试脚本验证核心功能：

```bash
python scripts/smoke_test.py
```

## Walk-Forward 验证（防过拟合）

使用 Walk-Forward 验证器在滑动窗口上优化并验证策略：

```python
from src.optimization.walk_forward import WalkForwardValidator, WalkForwardConfig
from src.optimization.objectives import MaximizeSharpe
from src.optimization.methods import GridSearch
from src.backtest.engine import BacktestConfig
from my_strategies import MyStrategy

config = WalkForwardConfig(train_period_days=180, test_period_days=30, n_splits=6)
validator = WalkForwardValidator(config)

result = validator.run(
    strategy_class=MyStrategy,
    data={"BTC/USDT": your_dataframe},  # DataFrame 需含 timestamp/open/high/low/close/volume
    param_space={
        "fast_period": {"type": "int", "min": 5, "max": 20, "step": 5},
    },
    objective=MaximizeSharpe(),
    search_method=GridSearch(),
    backtest_config=BacktestConfig(exchange="okx"),
    timeframe="15m",
)
print(result.is_robust, result.avg_test_sharpe)
```

## 测试

```bash
# 运行所有测试
pytest

# 运行单元测试
pytest tests/unit/

# 运行集成测试
pytest tests/integration/

# 生成覆盖率报告
pytest --cov=src tests/
```

## 贡献

这是一个个人项目，主要用于学习和实验。欢迎提 Issue 讨论想法，但不保证会接受 PR。

## 许可证

MIT License - 详见 [LICENSE](LICENSE)

## 免责声明

本项目仅供学习与研究使用。量化交易存在风险，使用本系统进行实盘交易造成的任何损失，作者不承担责任。请在充分理解策略逻辑和风险的前提下谨慎使用。

---

**注意**: 本系统设计用于中低频交易（15 分钟至 1 小时级别），不适合高频交易场景。
