        ...
        if self.type == "float":
            if self.step:
                ...
                return [
                    round(
                        self.min_value
                        + i * (self.max_value - self.min_value) / (n_samples - 1),
                        6,
                    )
        ...
        for name, spec in space.params.items():
            if spec.type == "float" and spec.step is None:
                param_values[name] = spec.generate_values(
                    n_samples=self.n_samples_per_float
                )
            else:
                param_values[name] = spec.generate_values()
        ...
        for name, spec in space.params.items():
            if spec.type in ("int", "float"):
                ...
                for i in range(self.n_samples):
                    low = (
                        spec.min_value
                        + i * (spec.max_value - spec.min_value) / self.n_samples
                    )
                    high = (
                        spec.min_value
                        + (i + 1) * (spec.max_value - spec.min_value) / self.n_samples
                    )
                    # 在区间内随机采样
                ...
                param_samples[name] = [
                    random.choice(values) for _ in range(self.n_samples)
                ]
