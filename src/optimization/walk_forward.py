...
class WalkForwardResult:
    ...
    def calculate_summary(self):
        ...
        # 平均指标
        self.avg_train_sharpe = sum(s.train_metrics.sharpe_ratio for s in valid_splits) / n
        self.avg_test_sharpe = sum(s.test_metrics.sharpe_ratio for s in valid_splits) / n
        
        self.avg_train_return = sum(s.train_metrics.total_return for s in valid_splits) / n
        self.avg_test_return = sum(s.test_metrics.total_return for s in valid_splits) / n
        
        # 衰减率
        if self.avg_train_sharpe != 0:
            self.sharpe_decay = (self.avg_train_sharpe - self.avg_test_sharpe) / abs(self.avg_train_sharpe)
        
        if self.avg_train_return != 0:
            self.return_decay = (self.avg_train_return - self.avg_test_return) / abs(self.avg_train_return)
        ...
        # 稳健性判断
        self.is_robust = (
            self.sharpe_decay < 0.3 and  # 夏普衰减 < 30%
            self.avg_test_sharpe > 0 and  # 测试期夏普 > 0
            self.parameter_stability > 0.7  # 参数稳定性 > 70%
        )
...
class WalkForwardValidator:
    ...
    def generate_splits(
        ...
        splits = []
        period_length = (
            timedelta(days=self.config.train_period_days) +
            timedelta(days=self.config.test_period_days) +
            timedelta(days=self.config.gap_days)
        )
        
        # 计算每个分割的起始点
        total_days = (data_end - data_start).days
        step_days = (total_days - self.config.train_period_days - self.config.test_period_days) // max(1, self.config.n_splits - 1)
    ...
    def run(
        ...
        Returns:
            WalkForwardResult
        """
        from src.optimization.engine import OptimizationEngine, OptimizationConfig
        ...
        for i, (train_data, test_data) in enumerate(splits_data):
            split = WalkForwardSplit(
                split_id=i,
                train_start=train_data.index.min() if isinstance(train_data.index, pd.DatetimeIndex) else train_data['datetime'].min(),
                train_end=train_data.index.max() if isinstance(train_data.index, pd.DatetimeIndex) else train_data['datetime'].max(),
                test_start=test_data.index.min() if isinstance(test_data.index, pd.DatetimeIndex) else test_data['datetime'].min(),
                test_end=test_data.index.max() if isinstance(test_data.index, pd.DatetimeIndex) else test_data['datetime'].max(),
            )
