...
class HistoryFetcher:
    ...
    BINANCE_TF_MAP = {
        ...
        "12h": "12h", "1d": "1d", "3d": "3d", "1w": "1w", "1M": "1mo",
    }
    ...
    def _parse_binance_klines(self, content: bytes, is_zip: bool = True) -> pd.DataFrame:
        """解析 Binance K 线数据"""
        try:
            if is_zip:
    ...
    async def _download_daily_fallback(
        ...
        for day in range(1, days_in_month + 1):
            ...
            url = f"{BINANCE_DATA_VISION_URL}/{daily_base}/{symbol}/{interval}/{filename}"
    ...
    async def download_range(
        ...
        if skip_existing:
            pending = self.checkpoint.get_pending_periods(
                self.exchange, symbol_upper, timeframe,
        ...
        for year, month in pending:
            df, is_new = await self.download_month(
    ...
    async def download_and_save(
        ...
        # 统计跳过的
        completed = self.checkpoint.get_completed_periods(
            self.exchange, symbol_upper, timeframe
        )
        stats.skipped_months = stats.total_months - stats.completed_months - stats.failed_months
