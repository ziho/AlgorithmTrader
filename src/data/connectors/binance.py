...
class BinanceConnector:
            ...
            try:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=60)) as response:
                    if response.status == 200:
                        ...
                        all_data.append(df)
                        downloaded_months += 1
                        logger.debug("binance_month_downloaded", month=year_month, rows=len(df))
                        
                    elif response.status == 404:
                        # 尝试日级别数据
                        daily_df = await self._download_daily_klines(
                            session, binance_symbol, interval, current, 
                            min(current.replace(month=current.month % 12 + 1, day=1) - timedelta(days=1), end_date),
                            base_path
                        )
    ...
    async def _download_daily_klines(
        ...
        while current <= end_date:
            ...
            url = f"{BINANCE_DATA_VISION_URL}/{base_path}/{symbol}/{interval}/{filename}"
