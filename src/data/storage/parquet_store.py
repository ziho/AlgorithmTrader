        ...
        try:
            ...
            if len(result) > 0:
                ...
                if min_ts and max_ts:
                    # 转换为 Python datetime
                    return (min_ts.replace(tzinfo=timezone.utc), max_ts.replace(tzinfo=timezone.utc))
            return None
        except Exception:
            return None
    
    def list_symbols(self, exchange: Optional[str] = None) -> list[Symbol]:
        """
        列出存储的所有交易对
        ...
        for exchange_dir in search_path.iterdir():
            ...
            for symbol_dir in exchange_dir.iterdir():
                ...
                if len(parts) == 2:
                    try:
                        symbol = Symbol.from_internal(f"{exchange_name}:{parts[0]}/{parts[1]}")
